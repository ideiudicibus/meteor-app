/**
 * helpers and clicks for Stringpe
 */



Template.playerProfileWithDatas.helpers({  
...
Template.playerProfileWithDatas.onRendered(()=>{
 fetchData( { player: Template.instance().data._id}, Template.instance() );

})

Template.playerProfileWithDatas.onCreated( () => {
...
Template.playerProfileWithDatas.events({
  'click [data-service]' ( event, template ) {
    ...
    template.checkout.open({
      ...
      bitcoin: false,
      currency: "eur",
      email:Meteor.user().emails[0].address!=undefined?Meteor.user().emails[0].address:""
    });
  }
});